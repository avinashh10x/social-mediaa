...
async function cacheThumbnail(url: string): Promise<string> {
  if (!url) return "";
  try {
    ensureThumbDir();
    ...
    if (buffer.length < 1000) return ""; // too small, probably an error page
  ...
  try {
    ...
    await runWithConcurrency(allTopVideos, VIDEO_CONCURRENCY, async (video) => {
      ...
      try {
        ...
        // Cache thumbnail while URL is fresh
        const thumbHash = await cacheThumbnail(video.thumbnail);
        ...
        const videoRecord: Video = {
          id: uuid(),
          link: video.postUrl,
          thumbnail: thumbHash ? `cached:${thumbHash}` : video.thumbnail,
          creator: video.username,
          views: video.views,
          likes: video.likes,
